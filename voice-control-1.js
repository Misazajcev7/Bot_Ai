 ...
 *
 * PATCH NOTE (for Part 5 integration):
 * The only functional change from the original Part 4 file is the final
 * `module.exports` line — it now also exports `speak`, since vision.js and
 * the unified bot.js router both call it directly for TTS output. Nothing
 * else in this file was touched. bot.js does NOT call setupVoiceControl()
 * anymore (it implements its own unified chat/console routing — see bot.js's
 * header comment), so the listener-attaching functions below are unused by
 * the assembled bot, but are left intact in case you want to run this file
 * standalone again.
 *
 * Requires:
 *   npm install say
...
module.exports = { setupVoiceControl, speak };
